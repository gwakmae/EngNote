...
// ═══ 직접 질문 프롬프트 (단어·구·문장 모두 이 하나로) ═══
const MANUAL_PROMPT_PLACEHOLDER = '(여기에 단어, 구, 또는 문장을 붙여넣는다)';

const MANUAL_PROMPT = `너는 영어 교사다. 나는 영어를 학습하는 한국인이다.
아래 [입력]을 분석해줘. 입력은 단어일 수도, 구(숙어·표현)일 수도, 문장일 수도 있다.

[판단 방법]
- 입력이 한 단어면: 그 단어를 분석한다.
- 입력이 여러 단어의 덩어리(구·숙어)면: 그 표현 전체를 하나로 분석한다.
- 입력이 문장이면: 문장 전체를 분석한다.
- 입력에 @@ 표시가 있으면: 그 부분이 내가 모르는 곳이므로 그 부분을 중심으로 분석한다.

[분석 형식]
1. 입력이 문장이면: 전체 문장의 자연스러운 한국어 해석을 먼저 적는다.
2. 단어·표현 분석:
   ...
   - 사전적 의미를 나열하지 말고, 문맥에서 쓰이는 그 뜻으로 설명한다.
   - 문장 안에서의 역할(품사, 무엇을 꾸미는지, 어떤 덩어리에 속하는지)을 짚어준다.
3. 입력이 문장이면: 문장 구조를 큰 틀부터 하향식으로 분석한다.
4. 재활용할 만한 문장 패턴·구문이 보이면 그것도 짚어준다.

[앱 등록용 데이터 - 반드시 준수]
분석이 끝난 후 맨 마지막에 아래 블록들을 출력한다.
- 형식을 절대 바꾸지 않는다. 마크다운, 이모지, 부연설명 금지.

(1) 단어 블록 — 입력에서 배울 만한 단어·숙어가 있는 경우 (입력 자체가 단어/구면 그것을 등록)
- 하나당 "---"로 구분
- 필드: en(영어 원문), pron-us(미국 발음, 어려운 단어만), pron-uk(영국 발음, 다를 때만), ko(한국어 뜻), ctx(원문 문장 — 입력이 문장일 때만, 단어·구만 입력했으면 생략)
- 활용형이면 표제어(동사 원형 등)를 쓴다.
...
(2) 패턴 블록 — 재활용 가능한 문장 틀이 있는 경우 (없으면 생략)
- 하나당 "---"로 구분
- 필드: pattern(빈자리는 ~로 표시), ko(설명), ex(예문 — 입력이 문장이면 그 문장 포함)
